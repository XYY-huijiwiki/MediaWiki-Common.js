...
await page.goto("https://xyy.huijiwiki.com/index.php?title=%E7%89%B9%E6%AE%8A:%E7%94%A8%E6%88%B7%E7%99%BB%E5%BD%95");
// login
dev && console.log(process.env.WIKI_USERNAME, process.env.WIKI_PASSWORD);
await page.type("#wpName1", process.env.WIKI_USERNAME);
await page.type("#wpPassword1", process.env.WIKI_PASSWORD);
...
// edit
let gh_repository = process.env.GITHUB_REPOSITORY;
let gh_actor = process.env.GITHUB_ACTOR;
let gh_sha = process.env.GITHUB_SHA;
console.log(gh_repository, gh_actor, gh_sha);
let contentPrefix = Mustache.render(fs.readFileSync("sync/warning.txt", "utf-8"), {
    gh_repository,
});
let content = fs.readFileSync("dist/common.js", "utf-8");
await page.waitForNavigation();
await page.evaluate(async (dev, gh_repository, gh_actor, gh_sha, contentPrefix, content) => {
...
}, dev, gh_repository, gh_actor, gh_sha, contentPrefix, content);
// end
await browser.close();
